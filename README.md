...
> código-fonte no export mais recente. A ronda mais recente foi um "Master Hardening & Release
> Gate" — não uma auditoria nova, mas a CORRECÇÃO directa de problemas específicos pendentes de
> uma auditoria anterior, executada em duas fases, cada uma com testes automatizados dedicados
> provando a correcção (141 testes no total desta ronda, todos a passar; `npm run lint`: 0 erros,
> 241 avisos — baseline inalterada).
>
> **Fase 1 — dois problemas financeiros críticos, confirmados por leitura directa do código:**
> **(1) Autorização da geração em cadeia** (`_planMode`/`_sectionMode`, usados por "Trabalho
> Escolar"/"Plano de Negócios"): o bloco de autenticação em `api/generate-document.js` tinha
> `if (!isChainCall && !isPreview)` — ou seja, qualquer pedido de cadeia SALTAVA a verificação de
> JWT por completo, gerando conteúdo de IA real, ilimitado e gratuito, sem sessão válida. Corrigido:
> JWT passa a ser obrigatório em todos os modos (excepto a pré-visualização, intencionalmente
> anónima); novo sistema `generation_jobs` (`migration_v68_generation_jobs.sql`) exige um job válido,
> ligado ao utilizador autenticado e com expiração, em cada `_sectionMode` — falha FECHADA (503) se
> a infra-estrutura de jobs estiver indisponível, nunca aberta. **(2) Custo controlado pelo
> cliente:** `api/_services/account.js` determinava o valor a debitar a partir de
> `parseInt(body?.cost)`, só validado contra um intervalo 1-10 sem qualquer ligação ao
> `documentType` real — um pedido directo podia declarar um serviço caro (ex.: "procuração", 3
> créditos) e pagar menos. O mesmo valia para templates pagos do marketplace. Corrigido com
> `api/_lib/pricingRegistry.js` — fonte única de verdade do preço por `documentType`, incluindo
> `templates_custom.credit_cost` para templates pagos; `body.cost` deixa de ser usado na cobrança
> (mantido, sem alteração, apenas na via de reembolso — risco residual distinto, documentado como
> tal). Testes: `generation-chain-security.test.js` (9), `cost-tampering.test.js` (11+1).
>
> **Fase 2 — dois problemas de integridade financeira adicionais, também confirmados por leitura
> directa do código:** **(3) Dupla atribuição de créditos no "Avulso":** `_createAvulsoAccount()`
> (`api/_services/payments.js`) escrevia `credits:X` directamente no perfil recém-criado — e os
> DOIS chamadores (aprovação automática por IA de visão, e por SMS M-Pesa reencaminhado) chamavam
> DEPOIS `add_credits(user, X)`, que é aditivo (`credits = credits + amount`). Resultado real: uma
> compra de X créditos ficava creditada a 2X, em TODO pagamento avulso aprovado automaticamente —
> não uma vulnerabilidade explorável, um bug de produção activo. Corrigido: `credits:0` explícito
> na criação (a conta nasce com 3 créditos por omissão via o trigger `on_auth_user_created`, tinha
> de ser zerado, não só omitido); `add_credits()` passa a ser a única fonte de verdade — e agora
> também grava correctamente em `credit_ledger`, o que a escrita directa antiga nunca fazia.
> Encontrada e corrigida no mesmo âmbito: uma condição de corrida em `handleConfirmAvulso` (modo
> normal, `api/admin/index.js`) — a transacção só era reivindicada DEPOIS de já ter criado a conta
> (ao contrário do endpoint irmão, que já reivindicava atomicamente primeiro), permitindo 2 contas
> e créditos em dobro em confirmações simultâneas. **(4) Previsibilidade do custo do "Trabalho
> Escolar":** existiam DOIS mecanismos de custo independentes e desalinhados — um cálculo por
> páginas (`dynamicCostPerPage:5`, "1 crédito a cada 5 páginas") para a cobrança inicial mostrada
> ao utilizador, e a cobrança progressiva por caracteres já existente (`CHARS_PER_EXTRA_CREDIT`,
> `LongDocumentEngine.js`) durante a geração. Um trabalho de 20 páginas mostrava/cobrava 4 créditos
> à partida, mas o total real rondava as 20. Corrigido com fonte única: `LongDocumentEngine.
> estimateCredits()`, derivada da MESMA constante que já determina a cobrança real, usada tanto
> para o valor mostrado ("≈N créditos no total") como internamente — a cobrança inicial passou a
> ser sempre 1 crédito fixo, o resto vem só da progressão real (nunca dois números a cobrar pela
> mesma coisa). Efeito colateral detectado e corrigido no processo: a correcção da Fase 1 (P20)
> tinha quebrado sem querer o custo dinâmico de "transcricao" (Digitalizar Documento, cobra por
> página OCR) — resolvido com uma excepção explícita e documentada (`CLIENT_ESTIMATED_SERVICES`),
> não uma vulnerabilidade nova, uma limitação pré-existente (custo de OCR só existe no cliente)
> preservada tal como estava. Testes: `avulso-credit-integrity.test.js` (4),
> `confirm-avulso-race.test.js` (3), `long-document-cost-estimate.test.js` (11, cobrindo 1, 5, 6,
> 10, 11, 15, 20 e 30 páginas conforme pedido).
>
> **Fase 3 — P1.4 (auditoria `SECURITY DEFINER`) + P2 (inventário de migrações):** inventário
> completo de todas as 20 funções `SECURITY DEFINER` únicas existentes (deduplicadas por nome,
> mantendo só a definição mais recente de cada uma). A `migration_v67` já tinha trancado 10; esta
> fase cobre as 6 que sobravam, cada uma com veredicto próprio — `is_admin_jwt()` teve de manter
> `EXECUTE` para "authenticated" (é usada dentro das próprias políticas RLS de admin, revogar
> quebraria o painel inteiro), só ganhando `search_path=''`. **Achado novo, fora do plano
> original:** `_consume_credit_ledger()` tinha `GRANT EXECUTE ... TO authenticated` — qualquer
> utilizador podia esvaziar o `credit_ledger` de OUTRA conta directamente via RPC; revogado.
> `expire_credit_batches()`/`cleanup_old_metrics_events()` só tinham `GRANT` (aditivo, nunca
> revoga o `EXECUTE` que PUBLIC ganha por omissão) sem `REVOKE` — continuavam abertas a qualquer
> pessoa; corrigido. Nova migração: `migration_v69_security_definer_search_path.sql`. Para o
> inventário de migrações (P2): nova subsecção 6.3.1 documenta os 12 ficheiros avulsos fora da
> cadeia numerada (tipo/objectivo/dependências/estado), incluindo um achado real —
> `transactions.sql` define essa tabela com um esquema INCOMPATÍVEL com o código actual (`id
> TEXT` em vez de `UUID`, sem `reference_id`/`confirmed_by`) — inofensivo só por coincidência
> (`CREATE TABLE IF NOT EXISTS` como no-op se `schema.sql` já tiver corrido primeiro). Nenhum
> ficheiro apagado ou alterado nesta fase, só documentado.
>
> **Fase 4 — fraude em comprovativos de pagamento (Payment Fraud):** `verifyReceiptInternal()`
> (`api/_services/payments.js`) calculava o hash anti-duplicação com
> `crypto.createHash('sha256').update(imageBase64.slice(0, 5000))` — só os primeiros 5000
> caracteres da STRING base64 (~3.7KB dos bytes reais), uma fracção mínima de qualquer fotografia
> real. Risco confirmado: alterar um único byte da imagem DEPOIS dessa fracção inicial (recomprimir,
> editar EXIF, recortar a margem) produzia um hash diferente para o que é, na prática, o MESMO
> comprovativo — permitindo reutilizar um único pagamento real várias vezes para reclamar créditos
> repetidamente, contornando a verificação `transactions?receipt_hash=eq....&status=eq.completed`.
> Corrigido: hash do binário real completo da imagem (`Buffer.from(imageBase64, 'base64')`), não da
> string base64 nem de um prefixo dela. Revisão à arquitectura de validação em torno deste ponto
> (secção 15 do plano de hardening, "AI confidence não pode ser a única autoridade"): confirmado por
> leitura que a decisão de aprovação automática já combinava confiança da IA (`confidence >= 0.85`)
> com 5 verificações determinísticas independentes (referência não reutilizada, janela de tempo,
> valor exacto do pacote, status explícito de sucesso, destinatário confirmado contra os números
> reais do MzDocs) — `allChecksPass`, com fallback explícito para revisão manual sempre que qualquer
> uma falhasse; esta parte já estava correcta antes desta ronda, não precisou de alteração. Teste
> novo: `payment-fraud.test.js` — reproduz o cenário exacto do exploit (duas imagens idênticas nos
> primeiros 6000 bytes mas diferentes depois: hashes diferentes após a correcção, seriam iguais
> antes). Suite completa: 144/144 testes, `npm run lint`: 0 erros (241 avisos, baseline inalterada).
>
> **Fase 5 — P1.7 (OCR/privacidade de imagem) + P1.8 (XSS armazenado no feedback):** resolvido.
> **P1.8:** o sink real e já explorável era o painel de moderação de avaliações no admin
> (`AdminApp.js`) — `user_name` (= `profiles.full_name` de quem avaliou) e `display_name`/`comment`
> iam para `innerHTML` sem escape correcto, permitindo que um nome de conta malicioso (`<img
> src=x onerror=...>`) corresse na sessão do **admin** só por o utilizador enviar qualquer avaliação
> (nem precisa de comentário). Corrigido: `escapeHtml()` em todos os campos de texto livre no admin;
> nova função partilhada `api/_lib/textSanitize.js` (`sanitizePlainText()`) usada em `handleFeedback`
> para nunca gravar `comment`/`display_name` com HTML; teste `feedback-security.test.js`. Residual
> menor e de baixo risco (a saída já está sempre escapada, isto é só defesa em profundidade extra):
> `full_name` continua sem limpeza de caracteres no signup (`api/auth/index.js`) nem na edição de
> perfil (`AuthManager.js`) — não corrigido nesta ronda, sinalizado para uma próxima.
> **P1.7:** confirmado que o pipeline principal de OCR (`SmartOCRService._compressImage()`) já
> redesenha a imagem num `<canvas>` antes de a enviar a qualquer IA, o que já elimina o EXIF
> (incluindo GPS) como efeito colateral — não era um bug, é uma protecção pré-existente que a
> auditoria original não tinha detectado. Corrigido: consentimento explícito obrigatório antes do
> primeiro uso da câmara/OCR (`OCRController.js`), e garantia de que nem a imagem nem os valores
> extraídos ficam alguma vez em logs do servidor (`api/_services/ocr.js`, redacção de campos
> sensíveis em `api/_lib/observability.js`); teste `ocr-privacy.test.js`. O que fica por resolver,
> sinalizado honestamente: mascarar/desfocar automaticamente uma região de BI/NUIT dentro da própria
> foto exigiria visão computacional dedicada, fora do que é viável nesta ronda.
>
> **Fase 6 — P1.9 (fact-check do SEO automático) + P1.11 (contabilidade económica no Admin):**
> **P1.9:** confirmado, por leitura de `api/_services/blog.js` (`_generateAndPublishArticle`), que
> um artigo gerado por IA era gravado com `published: true` e publicado como página estática no
> GitHub **imediatamente**, sem revisão nenhuma — mesmo para tópicos legais/fiscais/administrativos,
> onde um erro factual gerado por IA tem consequências reais (o próprio projecto já teve um exemplo
> do risco inverso — P1.10, a documentação interna do ISPC estava simplificada em excesso).
> Corrigido: nova detecção `_detectSensitiveTopic()` (palavras-chave legal/fiscal/administrativo —
> lei, imposto, ISPC, IRPS, NUIT, contrato, procuração, licença, direitos do trabalhador, etc.,
> verificada contra título+palavras-chave). Um tópico sinalizado é gravado com `published: false` +
> `needs_review: true` + `review_reason` (nova `migration_v71_blog_review_gate.sql`), NUNCA
> publicado automaticamente no site estático, e gera uma notificação para o admin
> (`admin_notifications`); o prompt de geração ganha também um aviso extra para não inventar
> números/prazos exactos nesses casos. A fila de agendamento (`blog_schedule_queue`) reflecte o novo
> estado (`status='needs_review'`, CHECK constraint alargado). Tópicos normais do catálogo (CV,
> cartas, trabalhos escolares, etc.) mantêm o fluxo 100% automático de sempre — a auditoria original
> recomendou explicitamente NÃO desligar o SEO programático, só ganhar revisão onde o risco é real.
> Admin panel (`AdminApp.js`, `filterBlog()`) passa a mostrar um destaque visual "🔎 Rever antes de
> publicar" nesses artigos, com o motivo em tooltip; corrigido também, no mesmo âmbito, um segundo
> XSS de baixo risco encontrado de raspão (título do artigo ia para `innerHTML` sem escape — só
> admin/IA podem criar artigos, não o público, mas a mesma disciplina de nunca confiar só na origem
> aplica-se aqui). Teste novo: `blog-seo-review-gate.test.js` (15 casos, títulos reais do catálogo
> confirmados como NÃO sensíveis, para não haver falsos positivos a mais). **P1.11:** novo
> sub-relatório `GET /api/admin?action=finance&sub=unit-economics` (`api/admin/index.js`), pensado
> para responder à pergunta da auditoria original: "quanto ganho por cada 1000 documentos?".
> Calcula, para um período: receita confirmada, créditos vendidos vs. **créditos realmente
> consumidos** (via `credit_logs`, ação `consume`) e documentos gerados; custos de IA (orçamento
> mensal configurado, prorateado pelos dias do período — não é medição real por pedido, assumido
> explicitamente na resposta), processamento de pagamento (nova `finance_payment_fee_pct`, taxa
> média ponderada configurável, por defeito 6,5%), comissão de afiliados (real, em regime de
> acumulação via `affiliate_commissions.created_at` no período — critério deliberadamente diferente
> do `sub=period-report`, que usa pagamentos efectivos, porque aqui o objectivo é medir a margem das
> vendas DESTE período, não o fluxo de caixa) e imposto (nova `fiscal_tax_rate_pct`, a 0% por
> defeito até confirmado com contabilista — mesmo estado honesto "a confirmar" já assumido no resto
> do projecto, P1.10). Devolve margem bruta total, por crédito vendido, por crédito consumido, por
> documento e **por 1000 documentos**, mais um bloco `assumptions` que documenta os limites de cada
> estimativa. Marketplace de templates tratado à parte (negócio diferente — royalty ao autor, não
> IA — misturado aqui distorceria a margem por crédito dos pacotes). UI nova no separador Finanças
> do admin (`admin.html`/`AdminApp.js`, cartão "🧮 Economia Unitária"), reaproveitando as mesmas
> datas do relatório de período já existente. Teste novo: `finance-unit-economics.test.js` (4
> casos, incluindo protecção contra divisão por zero quando não há documentos no período). Suite
> completa: 182/182 testes, `npm run lint`: 0 erros (241 avisos, baseline inalterada).
>
> **Com a Fase 6, fica coberta a lista completa P0.1–P1.12 da auditoria externa "Master Audit"
> (Set/2026)** — ver o histórico das 6 fases acima e na secção 15. Residuais conhecidos, de baixo
> risco, deixados para uma próxima ronda: `full_name` sem limpeza de caracteres no signup/edição de
> perfil (P1.8, ver acima); custo de "planonegocio" (Plano de Negócios) com rótulo fixo (2 créditos)
> sem estimativa dinâmica por não ter campo de páginas do qual a derivar; mascaragem visual de
> BI/NUIT dentro de uma foto de OCR (P1.7, exigiria visão computacional).
>
> A ronda anterior a esta corrigiu um bug real reportado por captura de ecrã — depois de criar
> conta, o "primeiro documento grátis" da v66 (`migration_v66_first_document_free.sql`,
> `grant_free_document()`) nunca chegava a ser concedido. Causa raiz confirmada por leitura do
> código: o mecanismo do **servidor** (`api/_services/account.js`) sempre esteve correcto — o bug
> estava inteiramente no **cliente**, em `CreditModel.canConsume()` (`Models.js`), que só olhava
> para `profiles.credits` (`credits >= n`); como uma conta nova nasce agora com 0 créditos (o
> benefício deixou de ser um saldo inicial desde a v66), o utilizador era bloqueado ali mesmo —
> antes de qualquer pedido chegar ao servidor — pelos dois pontos de verificação em
> `DocumentController.js` (`open()`/`generate()`) e por `FileConverter.js`; `grant_free_document()`
> nunca chegava a ser chamada. Também sintomático do mesmo bug: o badge de créditos no topo
> mostrava sempre "⚡0" e os cartões de serviço mostravam sempre "N créditos", mesmo com o
> benefício ainda por usar — nada na interface reflectia `CreditModel.freeDocsRemaining`, apesar de
> esse valor já ser sincronizado correctamente desde a v66 (`Services.js:syncUser()`). Corrigido
> sem alterar nenhuma lógica de servidor: `canConsume()` passa a considerar
> `freeDocsRemaining > 0` (novo getter `hasFreeDocument`) como suficiente para prosseguir, seja
> qual for o custo real do serviço; `Services.js`/`LongDocumentEngine.js` propagam a flag `free`
> devolvida por `/api/deduct-credit` até ao resultado da geração; `DocumentController.js`/
> `FileConverter.js` decrementam `freeDocsRemaining` localmente (novo `consumeFreeDocument()`,
> evento `freeDocsChanged`) assim que um documento grátis é usado, sem esperar pela próxima
> sincronização periódica (até 30s de atraso); badge do topo (`app.js`) e cartões de serviço
> (`index.html`) passam a mostrar "🎁 Grátis" enquanto o benefício não for usado; toast de registo
> (`AuthUI.js`) passa a confirmar explicitamente "Ganhou 1 documento grátis" (2 se via link de
> afiliado) — antes só o ecrã de confirmação por e-mail (caminho menos comum) mencionava isto.
>
> A ronda anterior a esta cobria uma auditoria externa completa ("Master Audit", Set/2026):
> **(a)** bloqueio de ~30 funções `SECURITY DEFINER` que nunca tinham tido
...
- **Migrações:** reorganizadas em Set/2026 (ver `supabase/README.md` e secção 13) — os antigos
  81 ficheiros soltos (`schema.sql` + `migration_v8_*` até `migration_v72_*`, mais um conjunto de
  ficheiros avulsos sem numeração: `EMERGENCIA_*`, `EXECUTAR_AGORA_*`, `migration_fix_*`,
  `migration_add_*`) foram **renomeados** (não reescritos — 0 diferenças de conteúdo,
  verificado byte-a-byte) para a convenção `<timestamp>_<nome>.sql` de `supabase/migrations/`,
  permitindo `supabase db reset` a partir do zero. Os originais ficam intocados em
  `supabase/_legacy_archive/` como histórico.
...
```
MzDocs-Pro/
├── api/                                # 10 Serverless Functions (Vercel Hobby, limite 12 — 2 de margem)
...
│   │   ├── aiRace.js                   # Corrida entre providers (tiers generoso/médio/reserva)
...
│   │   ├── blogResearch.js             # NOVO (Set/2026, Fase 6-ext): pesquisa em sites oficiais antes de gerar um artigo do blog — ver secção 4
│   │   ├── blogContentGuard.js         # NOVO (Set/2026, Fase 6-ext): 2ª camada de filtro — impede o artigo de mencionar IA/afirmar coisas que a MzDocs Pro não faz
│   │   ├── blogTemplate.js             # NOVO: template único partilhado das páginas estáticas do blog (elimina duplicação blog-cron ↔ admin manual)
│   │   ├── packages.js                 # Única fonte de verdade dos pacotes de créditos
│   │   ├── pricingRegistry.js          # Única fonte de verdade do custo por documentType/template pago
│   │   ├── piiRedaction.js             # Mascaragem de PII no texto (servidor)
│   │   ├── textSanitize.js             # sanitizePlainText() — nunca gravar HTML em campos de texto livre (feedback, etc.)
...
│   │   ├── parseMpesaSms.js            # Parser do SMS de confirmação da Vodacom M-Pesa (ver smsConfirm.js)
│   │   ├── phone.js                    # NOVO (Set/2026): normalização de nº moçambicano partilhada (+258...)
│   │   ├── whatsappCloud.js            # NOVO (Set/2026): cliente WhatsApp Cloud API (Meta) — ver api/whatsapp-webhook.js
│   │   ├── httpHelpers.js              # parseBody()/getAuthUser()/ORIGIN — extraído do antigo monólito api/misc.js
...
│   │   ├── smsConfirm.js               # NOVO: confirmação automática de pagamento via SMS M-Pesa reencaminhado (Telegram webhook OU HTTP directo) — corre em paralelo com a verificação por imagem, nunca a substitui
...
│   ├── whatsapp-webhook.js             # NOVO (Set/2026): recuperação de password via WhatsApp (ver secção 7) — function própria (não router) por precisar do corpo em bruto para verificar a assinatura HMAC da Meta
...
│   ├── convert.js                      # Conversão de ficheiros (CloudConvert, opcional; LibreOffice só em VPS própria)
│   └── misc.js                         # Router fino → api/_services/{payments,ocr,legal,blog,site,templates,affiliates,smsConfirm}.js
...
│
├── supabase/
│   ├── config.toml                              # NOVO — permite `supabase db reset` do zero
│   ├── migrations/                              # 81 migrações renomeadas (não reescritas), ordem
│   │                                             #   cronológica — ver supabase/README.md
│   ├── seed.sql                                 # corre automaticamente a seguir às migrações
│   ├── ops/promote_admin.sql                    # operação avulsa (fora do db reset)
│   ├── tests/database/                          # testes pgTAP (`supabase test db`)
│   └── _legacy_archive/                         # os 81 ficheiros ORIGINAIS, intocados (histórico)
│                                                 # ver secção 13 e supabase/README.md
│
...
│
├── pages/                          # 40 páginas SEO estáticas (geradas pelo admin via GitHub Contents API)
...
├── manifest.json · robots.txt · vercel.json · package.json (v11.1.0) · package-lock.json
├── .env.blog-example               # Exemplo de GITHUB_OWNER/GITHUB_REPO/GITHUB_TOKEN (ver secção 6)
├── b639f89ca02f968b334519652a68d401.txt   # Ficheiro de verificação de propriedade do domínio
│                                    #   (conteúdo = nome do próprio ficheiro; sem função no código)
└── scripts/inject-version.js
```

> 📎 **40 páginas SEO** — contagem confirmada directamente na pasta `pages/` (41 ficheiros no
> total, menos `pages/_template.html`, mantido como referência estática do formato de página —
> `api/admin/index.js` exclui-o explicitamente sempre que lista as páginas existentes; não é ele
> próprio uma página publicada).
...
### 6.3. Migrações Supabase — lista completa e actualizada (v8 → v71)
...
-- Autorização da geração em cadeia + custo controlado pelo cliente (v68–v69 — Set/2026, Master
-- Hardening & Release Gate v2, Fase 1 — mais recentes)
migration_v68_generation_jobs.sql                     -- tabela generation_jobs + RPCs
                                                       --   create_generation_job/
                                                       --   validate_generation_job — autorização
                                                       --   obrigatória de _planMode/_sectionMode
                                                       --   (ver secção 3 e 15); sem esta migração,
                                                       --   a geração em cadeia (Trabalho Escolar/
                                                       --   Plano de Negócios) responde 503 de
                                                       --   propósito (falha fechada)
migration_v69_security_definer_search_path.sql        -- P1.4: search_path explícito nas últimas
                                                       --   funções SECURITY DEFINER que ainda não
                                                       --   tinham (is_admin_jwt, cleanup_old_sessions,
                                                       --   approve/reject_template, increment_page_
                                                       --   view(s)/template_downloads); REVOKE EXECUTE
                                                       --   de PUBLIC/anon/authenticated em
                                                       --   cleanup_old_sessions/_consume_credit_ledger/
                                                       --   expire_credit_batches/
                                                       --   cleanup_old_metrics_events — nenhuma tinha
                                                       --   sido coberta pela v67; ver secção 7

-- Fact-check do SEO automático (v71 — Set/2026, Master Hardening & Release Gate v2, Fase 6 — mais
-- recente; não há v70 nesta cadeia — sem gap, simplesmente não foi criada nenhuma migração com
-- esse número)
migration_v71_blog_review_gate.sql                    -- colunas needs_review/review_reason em
                                                       --   blog_pages + CHECK alargado em
                                                       --   blog_schedule_queue (novo estado
                                                       --   'needs_review'); um artigo de IA sobre
                                                       --   tópico legal/fiscal/administrativo deixa
                                                       --   de ser publicado automaticamente — ver
                                                       --   secção 15, Fase 6
```
...
#### 6.3.1. Inventário dos ficheiros avulsos (P2 — Master Hardening, Set/2026, Fase 3)

Cronologia real determinada por leitura do conteúdo de cada ficheiro (comentários internos,
nomes de tabelas/funções que outras migrações já numeradas referenciam, e dependências
declaradas explicitamente) — não há histórico de Git disponível neste export para confirmar
datas exactas de commit. **Nenhum ficheiro foi apagado nem alterado nesta ronda** — o objectivo
aqui é só documentar, exactamente como pedido; ver secção 13 para a dívida técnica já conhecida
sobre este ponto.

| Ficheiro | Tipo | Objectivo | Dependências | Estado para uma instalação NOVA |
|---|---|---|---|---|
| `migration_add_email.sql` | Migração de schema | Acrescenta `profiles.email` (o próprio ficheiro diz "v3.3", pré-datando toda a cadeia numerada v8+) | Nenhuma | **Redundante** — `schema.sql` já cria `profiles.email` desde o início; só necessário para um ambiente muito antigo (pré-v3.3) que nunca correu `schema.sql` actualizado |
| `polices.sql` | Migração de schema (RLS base) | Políticas RLS iniciais de `profiles`/outras tabelas base | `schema.sql` | **Necessário** numa instalação nova, logo a seguir a `schema.sql` — mas as suas políticas de `profiles` (admin via `EXISTS(SELECT ... is_admin)`) são a CAUSA RAIZ da recursão infinita 42P17 corrigida por `migration_fix_rls_admin.sql` (ver linha abaixo) — correr sempre os dois, por esta ordem |
| `migration_add_is_blocked.sql` | Migração de schema | Acrescenta `profiles.is_blocked` | `schema.sql` | **Redundante** se a versão de `schema.sql` deste export já incluir a coluna (confirmar antes de correr — `ADD COLUMN IF NOT EXISTS` torna-o seguro de qualquer forma) |
| `EMERGENCIA_fix_recursion.sql` | Patch de emergência (não uma migração) | Desactiva RLS temporariamente para parar o erro 42P17 em produção, sem corrigir a causa | `polices.sql` (é o que está a corrigir) | **NÃO EXECUTAR numa instalação nova** — é um remendo temporário para parar uma incêndio em produção, substituído pela correcção definitiva de `migration_fix_rls_admin.sql`; incluído aqui só como registo histórico do incidente |
| `migration_fix_rls_admin.sql` | Migração de schema (correcção) | Correcção DEFINITIVA da recursão 42P17 — cria `is_admin_jwt()` (lê `auth.jwt()`, não a tabela `profiles`) e reescreve as políticas admin sem recursão | `polices.sql` | **Necessário** — é a versão correcta e permanente da correcção que `EMERGENCIA_fix_recursion.sql` só tapava temporariamente |
| `migration_fix_credits.sql` | Migração de schema | Cria `deduct_credits(UUID, INTEGER)` (débito de N créditos de uma vez) e simplifica `deduct_credit(UUID)` | `schema.sql` | **Necessário**, mas **já superseded** pela definição mais recente de `deduct_credits` em `migration_v52_credit_ledger.sql`/`migration_v67` — correr por ordem histórica, a versão final é a que fica activa (`CREATE OR REPLACE`) |
| `migration_monthly_credits.sql` | Migração de schema | Sistema de créditos mensais por plano — `grant_monthly_credits()`, uma das 3 funções chamadas directamente pelo browser (ver v67) | `schema.sql` | **Necessário** |
| `migration_temp_accounts.sql` | Migração de schema | Contas temporárias para pagamento avulso — `create_temp_account_for_avulso()` (ver correcção P19, secção 8/15) | **Depende explicitamente de `migration_monthly_credits.sql`** (dito no próprio ficheiro) | **Necessário** — usado activamente por `api/admin/index.js` (`handleConfirmAvulso`) |
| `EXECUTAR_AGORA_completo.sql` | Script de catch-up (consolidação ad-hoc, não uma migração) | Reaplica, num único ficheiro, colunas (`is_blocked`, `temp_ref`), `is_admin_jwt()` + políticas RLS (duplica `migration_fix_rls_admin.sql`) e `deduct_credit()` (duplica `migration_fix_credits.sql`) | — | **NÃO EXECUTAR numa instalação nova que já tenha corrido os ficheiros dedicados acima** — as suas `CREATE POLICY` (sem `DROP POLICY IF EXISTS` nem `CREATE OR REPLACE`) vão falhar com "policy already exists" se `migration_fix_rls_admin.sql` já tiver corrido antes. Histórico: foi um script de recuperação para um ambiente de produção que tinha ficado para trás, aplicado uma única vez |
| `EXECUTAR_promote_admin.sql` | Script operacional (não uma migração de schema) | Promove uma conta específica a admin e sincroniza `is_admin` para o `app_metadata` do JWT | Conta-alvo já ter sido criada | **NÃO faz parte da cadeia de instalação** — é uma operação pontual, específica de uma conta; para promover um novo admin hoje, usar a RPC `promote_to_admin()` (protegida desde a v67, ver secção 7), nunca este script |
| `corrigir-published-at.sql` | Script de backfill de dados (não uma migração de schema) | `UPDATE blog_pages SET published_at = created_at WHERE ...` para artigos já publicados antes de o admin gravar `published_at` correctamente | `migration_v26_blog_scheduling.sql` (cria a coluna) | **Não aplicável a uma instalação nova** — sem artigos antigos para corrigir, não há nada para este script fazer; seguro de ignorar |
| `transactions.sql` | Migração de schema — **INCOMPATÍVEL com o código actual** | Cria a tabela `transactions` com um esquema DIFERENTE do de `schema.sql` (`id TEXT` em vez de `UUID`, `user_phone` em vez de `phone_number`, sem `reference_id`/`confirmed_by`/`confirmed_at` — colunas que `api/admin/index.js` e `api/_services/payments.js` usam activamente hoje) | — | **NÃO EXECUTAR** — `CREATE TABLE IF NOT EXISTS` torna-o inofensivo SE `schema.sql` já tiver corrido primeiro (fica um no-op), mas nunca deve ser a fonte da tabela `transactions` numa instalação nova; a definição de `schema.sql` é a única compatível com o código actual. Ficheiro preservado só como registo histórico de uma tentativa anterior, provavelmente anterior ao próprio `schema.sql` |

**Conclusão do inventário (P2):** para `supabase db reset` produzir uma base de dados equivalente
à de produção a partir de um ambiente vazio, a ordem correcta é: `schema.sql` →
`polices.sql` → `migration_fix_rls_admin.sql` → `migration_fix_credits.sql` →
`migration_monthly_credits.sql` → `migration_temp_accounts.sql` → toda a cadeia numerada v8→v69
da secção 6.3 acima, por ordem. Os restantes ficheiros avulsos (`migration_add_email.sql`,
`migration_add_is_blocked.sql`, `EMERGENCIA_fix_recursion.sql`, `EXECUTAR_AGORA_completo.sql`,
`EXECUTAR_promote_admin.sql`, `corrigir-published-at.sql`, `transactions.sql`) não devem ser
corridos numa instalação nova pelas razões indicadas na tabela — mantidos no repositório apenas
como registo histórico, tal como pedido (não apagar migrações antigas indiscriminadamente).
Consolidar tudo isto num único `schema_CURRENT.sql`/baseline gerado directamente a partir do
Supabase Dashboard (Database → Schema) de um ambiente de produção real continua a ser a forma
mais fiável de obter uma cópia exacta — esta tabela é o melhor substituto possível só a partir
da leitura do código-fonte, sem acesso a esse Dashboard nem ao histórico de Git real.
...
> ✅ **Correcção P19 (Set/2026 — Master Hardening, Fase 2):** as contas "avulsas" mencionadas na
> limitação acima (`userId` NULL, criadas fora da transacção SQL) tinham um bug de dupla
> atribuição de créditos, confirmado por leitura directa do código: `_createAvulsoAccount()`
> (`api/_services/payments.js`) escrevia `credits:X` directamente no perfil, e os dois chamadores
> (aprovação automática por IA de visão, e por SMS) chamavam depois `add_credits(user, X)` —
> aditiva. Uma compra de X créditos ficava creditada a 2X, em todo pagamento avulso auto-aprovado.
> Corrigido: `_createAvulsoAccount()` cria sempre com `credits:0`; `add_credits()` passa a ser a
> única fonte de verdade do saldo final, e agora também grava em `credit_ledger` (o que a escrita
> directa antiga nunca fazia). Corrigida no mesmo âmbito uma condição de corrida equivalente em
> `handleConfirmAvulso` (modo "normal", `api/admin/index.js`) — a transacção passou a ser
> reivindicada atomicamente (`WHERE status=pending`) antes de qualquer criação de conta, tal como
> já acontecia em `confirm_payment_and_credit`. Testes: `avulso-credit-integrity.test.js`,
> `confirm-avulso-race.test.js` — ver secção 15 para o detalhe completo.
...
> ⚠️➡️✅ **Bug corrigido (Set/2026):** o mecanismo acima descrito sempre esteve correcto no
> **servidor**. Até Set/2026, porém, o **cliente** (`CreditModel.canConsume()` em `Models.js`) só
> verificava `profiles.credits`, e como `credits = 0` numa conta nova, bloqueava o utilizador antes
> de qualquer pedido chegar a `handleDeductCredit` — `grant_free_document()` nunca era chamada. Os
> pontos de bloqueio eram `DocumentController.js` (`open()`/`generate()`) e `FileConverter.js`; o
> badge de créditos (`app.js`) e os cartões de serviço (`index.html`) também nunca reflectiam
> `CreditModel.freeDocsRemaining`, mostrando sempre "0"/"N créditos". Corrigido — ver a entrada
> "Set/2026 (documento grátis)" na secção 15 para o detalhe completo por ficheiro.
...
**NOVO (Set/2026) — testes de base de dados:** `supabase/tests/database/00_schema_smoke_test.sql`
(pgTAP, 23 asserções, corre com `supabase test db`) é um teste de fumo separado do Jest —
confirma que, depois de um `supabase db reset` a partir do zero, as estruturas que já causaram
incidentes reais em produção continuam a existir (colunas críticas de `profiles`, RLS activo,
`is_admin_jwt()` sem recursão à própria tabela, trigger de signup). Não corre no CI actual (o
workflow não sobe Postgres/pgTAP) — corre manualmente por agora; ver `supabase/README.md`.
...
- ~~**Ficheiros de migração avulsos** (`EMERGENCIA_*`, `EXECUTAR_AGORA_*`, `migration_fix_*`,
  ...
  principal.~~ — **resolvido (Set/2026):** todos os 81 ficheiros (numerados e avulsos) foram
  ordenados e renomeados para `supabase/migrations/` (conteúdo SQL 100% preservado, verificado
  byte-a-byte — só o nome/pasta mudou). `EXECUTAR_promote_admin.sql` foi dividido em parte
  estrutural (migração) + parte operacional (`supabase/ops/promote_admin.sql`, com o e-mail
  pessoal original substituído por placeholder). Ver `supabase/README.md` para as decisões de
  ordem tomadas e os pontos que ainda vale a pena confirmar manualmente contra produção.
- **Duas migrações chamadas "v46"** coexistem no repositório por lapso de nomenclatura — não é a
  mesma migração corrida duas vezes, são dois ficheiros distintos, sem dependência entre si;
  ambas preservadas em `supabase/migrations/` com timestamps distintos, por isso já não há
  ambiguidade de ordem — mas o lapso de nomenclatura original mantém-se por não valer a pena
  renomear o conteúdo histórico.
- ~~**`migration_v31_marketing_purchase_attribution.sql`** foi assinalada em auditorias anteriores
  ...
  Supabase Dashboard se já foi aplicada antes de a tentar correr de novo.~~ — **confirmado
  (Set/2026):** o ficheiro está mesmo corrompido (100% bytes nulos, não é SQL). Substituído por
  um placeholder documentado em `supabase/migrations/`; sem referências no código da app
  (`api/`, `assets/`), por isso não parece estar a causar problemas visíveis hoje. Recuperar o
  schema real requer `supabase db pull` contra produção, se a funcionalidade ainda for desejada.
...
| Set/2026 (documento grátis) — bug real: "documento grátis" (v66) bloqueado no cliente antes de chegar ao servidor | Reportado com 2 capturas de ecrã (conta nova com 0 créditos: badge "⚡0" no topo, e ao tentar gerar um CV, "⚠️ Créditos insuficientes. Compre mais para continuar." — sem nunca chegar a chamar o servidor). Causa raiz confirmada por leitura do código: o mecanismo do servidor (`grant_free_document()`, `migration_v66`) sempre funcionou correctamente; o bug estava em `CreditModel.canConsume()` (`Models.js`), que só verificava `profiles.credits` — como uma conta nova nasce com 0 créditos desde a v66 (o benefício deixou de ser um saldo inicial), o utilizador era bloqueado no cliente, nos dois pontos de verificação em `DocumentController.js` (`open()`/`generate()`) e em `FileConverter.js`, sem o pedido chegar sequer a `/api/deduct-credit`. Corrigido, sem tocar em nenhuma lógica de servidor: **(1)** `canConsume()` passa a considerar `freeDocsRemaining > 0` (novo getter `CreditModel.hasFreeDocument`) suficiente para prosseguir, seja qual for o custo do serviço; **(2)** `Services.js`/`LongDocumentEngine.js` propagam a flag `free` devolvida por `/api/deduct-credit` até ao resultado da geração (`result.freeDocument`); **(3)** `DocumentController.js`/`FileConverter.js` decrementam `freeDocsRemaining` localmente assim que um documento grátis é usado (novo `CreditModel.consumeFreeDocument()`, evento `freeDocsChanged`), sem esperar pela próxima sincronização periódica (até 30s de atraso); **(4)** badge de créditos no topo (`app.js`) e cartões de serviço (`index.html`, `getCredits()`) passam a mostrar "🎁 Grátis" em vez de "N créditos"/"0" enquanto o benefício não for usado, re-renderizando ao vivo nos eventos `creditsChanged`/`freeDocsChanged`; **(5)** toast de registo (`AuthUI.js`, caminho de login automático — o mais comum) passa a confirmar explicitamente "Ganhou 1 documento grátis" (2 se o registo veio de um link de afiliado, `mz_ref` em `localStorage`) — antes só o ecrã de confirmação por e-mail (caminho menos comum) mencionava isto; **(6)** CTA da hero (`index.html`) e texto do convite de partilha (`DocumentController.js._showReferralCTA`) actualizados de "1 crédito" (mecanismo antigo, desligado desde a v66) para "documento(s) grátis". `DocumentEditor.js._renderFreeDocBanner()` (já existente, correcto) não precisou de alterações — passou a reflectir o estado real assim que `freeDocsRemaining` deixou de ficar preso a um valor desactualizado |
| Set/2026 (Master Hardening — Fase 1) — autorização da geração em cadeia + custo controlado pelo cliente | Execução directa de um plano de hardening externo ("Master Hardening & Release Gate v2"), não uma auditoria nova — correcção de problemas específicos, cada um confirmado por leitura directa do código antes de alterar qualquer coisa. **(1) P1.1 — autorização da geração em cadeia:** `api/generate-document.js` tinha `if (!isChainCall && !isPreview) { ...exigir JWT... }` — ou seja, `_planMode`/`_sectionMode` (usados por "Trabalho Escolar"/"Plano de Negócios") SALTAVAM a verificação de JWT por completo; `verifiedUserId` ficava igual ao `userId` enviado no corpo do pedido, nunca verificado. Qualquer pedido directo ao endpoint com `_planMode:true` ou `_sectionMode:true` gerava conteúdo de IA real, ilimitado e gratuito, sem sessão válida — o token já ia em `chainHeaders` desde o cliente, mas nunca era verificado no servidor. Corrigido: JWT obrigatório em todos os modos (excepto `_previewMode`, intencionalmente anónimo); novo sistema `generation_jobs` (`migration_v68_generation_jobs.sql`, tabela + RPCs `create_generation_job`/`validate_generation_job`, `SECURITY DEFINER SET search_path=''`) — `_planMode` cria um job ligado ao utilizador autenticado, `_sectionMode` exige esse `jobId` e o servidor confirma posse+validade (não expirado, não doutro utilizador) antes de gastar qualquer token de IA; falha FECHADA (503) se a RPC estiver indisponível, nunca aberta. `LongDocumentEngine.js` alterado para criar/propagar o `jobId` entre o plano e cada secção. **(2) P20 — custo controlado pelo cliente:** `api/_services/account.js` tinha `const rawCost = parseInt(body?.cost); const cost = VALID_COSTS.includes(rawCost) ? rawCost : 1` — só validava um intervalo (1-10), sem qualquer ligação ao `documentType` real; um pedido directo podia declarar `documentType: "procuracao"` (custo real 3) com `cost:1` e pagar menos. O mesmo valia para templates pagos do marketplace (`TemplatePicker.js` enviava só a `service_key`, não o UUID real do template). Corrigido com novo `api/_lib/pricingRegistry.js` — fonte única de verdade do preço por `documentType` (espelha `ServiceDefinitions.js`), incluindo `templates_custom.credit_cost` para templates pagos (via UUID real, agora enviado por `TemplatePicker.js`); `body.cost` deixa de ser usado na via de COBRANÇA (mantido sem alteração na via de REEMBOLSO — já mitigada por idempotência via `operationId`, `migration_v60` — risco residual distinto, fora do âmbito desta fase, não escondido). Testes novos: `generation-chain-security.test.js` (9 — sem token, sem job, job doutro utilizador, job expirado/RPC indisponível → sempre recusado ou 503), `cost-tampering.test.js` (11 — `cost=0/-1/999999/NaN/null`, `chargeType=extra_page` mal-usado, template pago). Suite completa: 122/122 testes, `npm run lint`: 0 erros (241 avisos, baseline inalterada) |
| Set/2026 (Master Hardening — Fase 2) — dupla atribuição no Avulso + previsibilidade do custo do Trabalho Escolar | **(3) P19 — dupla atribuição de créditos no "Avulso":** `_createAvulsoAccount()` (`api/_services/payments.js`) escrevia `credits:X` directamente no perfil via `update()` — e os DOIS chamadores (`verifyReceiptInternal`, aprovação automática por IA de visão; `smsConfirm.js`, confirmação automática por SMS M-Pesa reencaminhado) chamavam DEPOIS `add_credits(user, X)`, RPC aditiva (`credits = credits + amount`, `migration_v52_credit_ledger.sql`). Resultado real, confirmado por leitura do código: uma compra avulso de X créditos, aprovada automaticamente, ficava creditada a 2X — não uma vulnerabilidade explorável por um atacante, um bug de produção activo em todo pagamento avulso auto-aprovado. Corrigido: `credits:0` explícito na criação (o trigger `on_auth_user_created` já insere qualquer conta nova com `credits=3` por omissão — tinha de ser zerado, não só omitido, para `add_credits()` ser a única fonte de verdade do saldo final); como bónus, `add_credits()` grava correctamente em `credit_ledger`, o que a escrita directa antiga nunca fazia (contas avulso tinham sempre um ledger incompleto). Encontrada e corrigida no mesmo âmbito, mesma causa-raiz (dedução da lógica, não da auditoria original): uma condição de corrida em `handleConfirmAvulso` (modo "normal", `api/admin/index.js`) — a transição de estado da transacção só acontecia DEPOIS de já ter criado a conta temporária (ao contrário do endpoint irmão `handleConfirmPayment`, que já reivindicava a transacção atomicamente primeiro, `WHERE status=pending`); dois pedidos de confirmação quase simultâneos liam ambos `status==='pending'` antes de qualquer um escrever, e cada um criava a sua própria conta + créditos — corrigido com o mesmo padrão atómico do endpoint irmão, reivindicação movida para antes de qualquer criação. **(4) P1.2 — previsibilidade do custo do "Trabalho Escolar":** existiam DOIS mecanismos de custo independentes e desalinhados — `dynamicCostPerPage:5` ("1 crédito a cada 5 páginas", introduzido numa ronda anterior — ver linha "Ago/2026 (ronda seguinte)" acima) determinava a cobrança INICIAL mostrada no botão, e `CHARS_PER_EXTRA_CREDIT` (já existente desde a v3.0 de `LongDocumentEngine.js`) cobrava progressivamente MAIS créditos DURANTE a geração, conforme o texto realmente ia crescendo. Um trabalho de 20 páginas mostrava/cobrava 4 créditos à partida (20÷5), mas o total real rondava as 20 — exactamente a "dupla cobrança" descrita no plano de hardening. Corrigido com fonte única: novo `LongDocumentEngine.estimateCredits(pages)`, derivado da MESMA constante `CHARS_PER_EXTRA_CREDIT` que já determina a cobrança real (se o valor de calibração mudar, a estimativa acompanha automaticamente); a cobrança INICIAL de "trabalho"/"planonegocio" passou a ser sempre 1 crédito fixo (`SERVICE_COSTS`, `pricingRegistry.js`), o resto vem só da progressão já existente. UI (`Views.js`/`DocumentController.js`) mostra agora "1 crédito agora · ≈N no total", claramente rotulado como estimativa, com aviso explícito de que a geração continua de forma graciosa (documento parcial) se os créditos acabarem a meio — comportamento já existente, não alterado. **Efeito colateral detectado e corrigido no processo** (não fazia parte do plano original): a correcção de P20 (Fase 1) tinha quebrado sem querer o custo dinâmico de "transcricao" (Digitalizar Documento, cobra por página OCR fotografada — um valor que só existe no cliente, `docModel.ocrPageCount`, sem geração em cadeia); resolvido com nova `CLIENT_ESTIMATED_SERVICES` em `pricingRegistry.js` — excepção explícita e documentada que restaura o comportamento pré-existente (não uma vulnerabilidade nova, uma limitação já existente antes desta ronda, preservada tal como estava). **Não resolvido nesta fase, sinalizado honestamente:** "planonegocio" não tem campo de páginas no formulário (é sempre geração em cadeia, independentemente de tamanho) — mantém o rótulo fixo antigo (2 créditos) sem estimativa dinâmica, por não haver input do qual a derivar; risco residual menor. Testes novos: `avulso-credit-integrity.test.js` (4), `confirm-avulso-race.test.js` (3), `long-document-cost-estimate.test.js` (11 — 1, 5, 6, 10, 11, 15, 20 e 30 páginas, conforme pedido explicitamente no plano de hardening). Suite completa: 141/141 testes, `npm run lint`: 0 erros (241 avisos, baseline inalterada) |
| Set/2026 (Master Hardening — Fase 3) — inventário e hardening de funções SECURITY DEFINER + inventário de migrações | **(5) P1.4 — SECURITY DEFINER:** inventário completo feito por leitura directa de todas as 78 migrações (deduplicado por nome de função, mantendo só a definição MAIS RECENTE de cada uma — a que fica realmente activa via `CREATE OR REPLACE`): 20 funções `SECURITY DEFINER` únicas no total. A `migration_v67` já tinha revogado `EXECUTE` de PUBLIC/anon/authenticated em 10 delas; esta fase cobre as 6 que sobravam, com um veredicto próprio para cada uma em vez de aplicar a mesma correcção cegamente — `is_admin_jwt()` é usada DENTRO das políticas RLS de admin (`profiles_select_admin` etc., `migration_fix_rls_admin.sql`) e por isso TEM de continuar executável por "authenticated" (revogar quebraria o acesso admin ao painel inteiro) — só ganhou `search_path=''` (já só referia `auth.jwt()`, totalmente qualificado). **Achado novo, não listado explicitamente no plano** (secção 24 do plano de hardening: "se encontrares uma vulnerabilidade nova, não a ignores"): `_consume_credit_ledger(p_user_id, p_amount)` tinha `GRANT EXECUTE ... TO authenticated` — qualquer utilizador podia chamá-la directamente via RPC com o `p_user_id` de OUTRA pessoa e esvaziar os lotes de créditos ainda por gastar dessa conta no `credit_ledger` (não mexe em `profiles.credits` directamente, mas corrompe irreversivelmente a contabilidade/auditoria desse ledger para a vítima); confirmado por grep a todo `api/`/`assets/js/` que só é chamada internamente por `deduct_credits()` — revogado o `GRANT` a "authenticated", mantido só `service_role` (chamadas internas entre funções `SECURITY DEFINER` correm com os privilégios do DONO, não do utilizador original, por isso isto não quebra `deduct_credits()`). `expire_credit_batches()`/`cleanup_old_metrics_events()` só tinham `GRANT ... TO service_role` sem nenhum `REVOKE ... FROM PUBLIC` antes — `GRANT` é aditivo, nunca remove o `EXECUTE` que o Postgres concede a PUBLIC por omissão a qualquer função nova; ambas continuavam chamáveis por qualquer pessoa (expirar lotes de créditos de TODOS os utilizadores de uma vez, ou apagar telemetria, à vontade). `cleanup_old_sessions()` não tinha nem `search_path` nem `REVOKE` — confirmado por grep que nunca é chamada pelo browser nem pelo servidor, ficou reservada a `service_role`. `approve_template`/`reject_template`/`increment_page_view(s)`/`increment_template_downloads` já estavam correctamente trancadas pela v67 — só lhes faltava `search_path` explícito, acrescentado sem qualquer mudança de lógica (corpo confirmado igual à última versão de cada uma). `handle_new_user()` (trigger de `auth.users`) já tinha `search_path` e está estruturalmente protegida contra invocação directa via RPC pelo próprio motor do Postgres (funções `RETURNS TRIGGER` recusam ser chamadas fora do contexto de um trigger) — sem alteração necessária. Nova migração: `migration_v69_security_definer_search_path.sql`. **(6) P2 — inventário de migrações:** a secção 6.3 já cobria em detalhe a cadeia numerada v8→v67 (gaps v18/v19 e v58, duplicação de nome "v46", risco de export vazio na v31) — actualizada para v8→v69. Nova subsecção 6.3.1: inventário completo dos 12 ficheiros avulsos fora da cadeia numerada (`migração/tipo/objectivo/dependências/estado`, exactamente como pedido), com a cronologia real deduzida por leitura do conteúdo (não há histórico de Git disponível neste export). Achados: `EMERGENCIA_fix_recursion.sql` é um remendo temporário (desactiva RLS) já substituído pela correcção definitiva em `migration_fix_rls_admin.sql`; `EXECUTAR_AGORA_completo.sql` duplica essa mesma correcção com `CREATE POLICY` sem `DROP POLICY IF EXISTS` — corrê-lo depois do ficheiro dedicado causaria erro "policy already exists"; `transactions.sql` define a tabela `transactions` com um esquema INCOMPATÍVEL com o código actual (`id TEXT` em vez de `UUID`, sem `reference_id`/`confirmed_by`, colunas que `api/admin/index.js` usa activamente) — inofensivo só porque `CREATE TABLE IF NOT EXISTS` o torna um no-op se `schema.sql` já tiver corrido primeiro; `EXECUTAR_promote_admin.sql`/`corrigir-published-at.sql` são scripts operacionais/de backfill pontuais, não migrações de schema, não fazem parte de uma instalação nova. **Nenhum ficheiro foi apagado ou alterado** — só documentado, tal como pedido explicitamente; ordem de instalação recomendada para `supabase db reset` documentada no final da secção 6.3.1. Sem testes de código nesta fase (é trabalho de SQL/documentação) — validação feita por leitura cruzada de cada função com o código-fonte que a chama (`grep` a `api/`/`assets/js/`), não por `npm test` |
| Set/2026 (Master Hardening — Fase 4) — fraude em comprovativos de pagamento (hash de deduplicação) | **(7) Payment Fraud — hash do comprovativo:** `verifyReceiptInternal()` (`api/_services/payments.js`) calculava o hash anti-duplicação com `crypto.createHash('sha256').update(imageBase64.slice(0, 5000))` — só os primeiros 5000 caracteres da STRING base64 (cerca de 3.7KB dos bytes reais da imagem, decodificados) entravam no cálculo, uma fracção mínima de qualquer fotografia real (tipicamente 100KB-2MB). Risco confirmado por leitura do código: bastava alterar um único byte da imagem DEPOIS dessa fracção inicial — recomprimir o JPEG, editar metadados EXIF, recortar ligeiramente a margem — para obter um hash DIFERENTE para o que é, na prática, o MESMO comprovativo, contornando a verificação de duplicado (`transactions?receipt_hash=eq....&status=eq.completed&select=reference_id`) e permitindo reutilizar um único pagamento real várias vezes para reclamar créditos repetidamente. Corrigido: `crypto.createHash('sha256').update(Buffer.from(imageBase64, 'base64')).digest('hex')` — hash do BINÁRIO REAL completo da imagem, decodificado a partir do base64, não da string codificada nem de um prefixo dela; duas imagens só produzem o mesmo hash se forem byte a byte idênticas. Confirmado por grep a todo `api/`/`assets/js/` que o cliente (`PaymentController.js`) já envia o base64 sem o prefixo `data:image/...;base64,` (`dataUrl.split(',')[1]`) antes de chegar a este ponto — a mesma suposição que `analyzeImage()` já fazia com sucesso, por isso a correcção não introduz nenhuma regressão de formato. **Revisão da arquitectura de validação em torno deste ponto** (secção 15 do plano de hardening — "AI confidence não pode ser a única autoridade"): confirmado por leitura completa de `verifyReceiptInternal()` que a decisão de aprovação automática já não dependia só da confiança da IA — `confidence >= 0.85 && allChecksPass`, com `allChecksPass` a exigir simultaneamente 5 verificações determinísticas independentes: referência não reutilizada noutra transacção completa, janela de tempo (máx. 60 min), valor exacto do pacote esperado (tolerância de 1 MZN), status explícito de sucesso, e destinatário confirmado contra os números reais de recepção do MzDocs (nunca aprovado "às cegas" se o número extraído ficar vazio/curto) — com fallback explícito e detalhado para revisão manual (`_markReviewNeeded`, com o motivo exacto de cada verificação que falhou) sempre que qualquer uma destas condições não se verificasse. Esta parte da arquitectura já estava correcta antes desta ronda e não precisou de nenhuma alteração — o único defeito real encontrado, em toda a secção 15 do plano de hardening, foi o hash truncado. Teste novo: `payment-fraud.test.js` (3 testes) — reproduz directamente o cenário do exploit (duas imagens construídas para partilhar os primeiros 6000 bytes/8000+ caracteres base64 mas diferentes depois: hashes DIFERENTES depois da correcção, seriam IGUAIS com o código antigo — comparação explícita contra o hash do bug antigo incluída no teste), mais um teste de determinismo (mesma imagem → mesmo hash sempre, essencial para a deduplicação funcionar). Suite completa: 144/144 testes, `npm run lint`: 0 erros (241 avisos, baseline inalterada) |
...
| Migrações Supabase | até `migration_v71_blog_review_gate.sql` (gaps reais em `v18`/`v19` e `v58`), mais ficheiros avulsos não numerados |
...
| Primeiro documento grátis | desde a v66 (`free_documents_used`) — 1 conta normal / 2 via afiliado, nunca em `profiles.credits`, visível no painel admin e na interface do utilizador (badge/cartões "🎁 Grátis") desde Set/2026 |
...
| Testes (Jest, CI) | 21 suites / 182 testes, ≈ 3.146 linhas |
