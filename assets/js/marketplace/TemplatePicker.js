...
/* ── Folha A4 individual — mostra documento COMPLETO com scroll ── */
.tpl-page{
  background:#fff;
  width:100%;
  max-width:560px;
  /* Removido aspect-ratio:210/297 — limitava a 1 página e cortava o resto */
  min-height:200px;
  border-radius:3px;
...
/* iframe cresce conforme o conteúdo real do documento */
.tpl-page iframe{
  width:210mm;
  height:297mm;      /* altura mínima 1 página — JS pode aumentar após load */
  border:none;
  display:block;
...
${tpl.css || ''}
  ...
  _scalePage(pageEl, iframe) {
    const containerW = pageEl.parentElement?.clientWidth || pageEl.clientWidth || 320;
    const a4Wpx = 210 * 3.7795;  // 210mm → ~794px
    const scale  = containerW / a4Wpx;

    iframe.style.transform       = `scale(${scale})`;
    iframe.style.transformOrigin = 'top left';
    pageEl.style.aspectRatio     = '';  // remover aspect-ratio fixo

    // Ajustar altura ao conteúdo real (documento pode ter múltiplas páginas)
    const applyHeight = () => {
      try {
        const contentH = iframe.contentDocument?.documentElement?.scrollHeight
                      || iframe.contentDocument?.body?.scrollHeight
                      || 0;
        const scaled = contentH > 0 ? contentH * scale : 297 * 3.7795 * scale;
        pageEl.style.height = Math.ceil(scaled) + 'px';
        pageEl.style.width  = Math.ceil(a4Wpx * scale) + 'px';
      } catch (_) {
        // cross-origin fallback: 1 página A4
        pageEl.style.height = Math.ceil(297 * 3.7795 * scale) + 'px';
        pageEl.style.width  = Math.ceil(a4Wpx * scale) + 'px';
      }
    };

    applyHeight();
    // Segunda passagem após render completo (imagens, fontes, etc.)
