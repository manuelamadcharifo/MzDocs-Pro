...
USING (EXISTS (
  ...
  WHERE id = auth.uid() AND is_admin = TRUE
    ...
    async _loadBlogQueue() {
        const tbody = document.getElementById('blogQueueTable');
        if (!tbody) return;
        try {
            const token = await this._getAdminToken();
            ...
            if (!items.length) {
                tbody.innerHTML = '<tr><td colspan="5" style="text-align:center;padding:16px;color:#94a3b8;">Sem artigos agendados. Use "📅 Agendar em Massa" ou active a "🤖 Geração Automática".</td></tr>';
                return;
            }
            tbody.innerHTML = items.map(it => {
                ...
                const statusBadge = {
                    pending:   '<span style="background:#FEF9C3;color:#713F12;font-size:11px;font-weight:700;padding:2px 8px;border-radius:10px;">⏳ Agendado</span>',
                ...
                const actions = it.status === 'pending'
                    ? `<button class="btn-ghost" style="font-size:12px;color:#ef4444;" onclick="adminApp._deleteBlogQueueItem('${it.id}')">🗑️ Remover</button>`
                    : (it.error_note ? `<span style="font-size:11px;color:#991B1B;" title="${(it.error_note||'').replace(/"/g,'&quot;')}">ⓘ erro</span>` : '—');
                return `<tr>
            ...
            tbody.innerHTML = '<tr><td colspan="5" style="text-align:center;padding:16px;color:#ef4444;">Erro: ' + err.message + '</td></tr>';
        }
    }
    ...
    openBlogBulkSchedule() {
        ...
        this.showModal(`
            ...
            <p class="modal-sub">Cole um título por linha (até 200). Cada um será gerado por IA e publicado automaticamente na data agendada — lendo sempre os artigos já existentes para não repetir conteúdo.</p>
            <div class="modal-field">
    ...
    async _saveBlogAutogenSettings() {
        ...
        try {
            ...
            this._notify(enabled ? `✅ Geração automática activada (a cada ${intervalDays} dias)` : '✅ Geração automática desactivada', 'success');
        } catch (err) {
            this._notify('❌ ' + err.message, 'error');
        }
    }
    ...
    async _loadTemplates(status = 'pending') {
        ...
        try {
            const { data, error } = await this.supabase
                ...
                .select('id, template_name, description, template_html, template_css, service_type, status, is_public, created_at, downloads, user_id, rejection_note, credit_cost, is_featured')
                .eq('status', status)
                .order('created_at', { ascending: false })
                .limit(50);
            ...
            container.innerHTML = data.map(tpl => {
                ...
                return `<div style="background:#fff;border:1.5px solid #e2e8f0;border-radius:16px;overflow:hidden;box-shadow:0 2px 12px rgba(0,0,0,.07)">
                  ...
                  <div style="padding:14px">
                    ...
                    <!-- CORRIGIDO (auditoria, ponto 3): a API de gestão de preço
                         (api/admin/index.js → handleTemplates) já existia, mas
                         não havia nenhuma UI para o admin a usar — definir o
                         preço só era possível chamando a API directamente.
                         Este bloco usa update directo no Supabase, mesmo
                         padrão já usado por _approveTemplate/_rejectTemplate
                         nesta mesma função. -->
                    <div style="display:flex;align-items:center;gap:8px;margin-bottom:10px;background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;padding:8px 10px">
    ...
    async _approveTemplate(id) {
        ...
        try {
            const { error } = await this.supabase
                .from('templates_custom')
                .update({ status: 'approved', is_public: true, approved_at: new Date().toISOString(), rejection_note: null })
                .eq('id', id);
            if (error) throw error;
    ...
    async _rejectTemplate(id) {
        ...
        try {
            ...
            const { error } = await this.supabase
                .from('templates_custom')
                .update({ status: 'rejected', is_public: false, rejection_note: note || null, rejected_at: new Date().toISOString() })
                .eq('id', id);
            if (error) throw error;
    ...
    // CORRIGIDO (auditoria, ponto 3): permite ao admin definir manualmente
    // o preço em créditos e o destaque de cada template da galeria — antes
    // só era possível via chamada directa à API (api/admin/index.js →
    // handleTemplates), sem nenhuma interface no painel.
    async _saveTemplatePricing(id) {
        ...
        try {
            const { error } = await this.supabase
                .from('templates_custom')
                .update({ credit_cost: cost, is_featured: !!featInput?.checked, updated_at: new Date().toISOString() })
                .eq('id', id);
            if (error) throw error;
            this._notify(cost > 0 ? `✅ Preço definido: ${cost} crédito(s).` : '✅ Template marcado como gratuito.');
        } catch (err) {
...
<body>${data.template_html || '<p>Sem conteúdo HTML</p>'}</body></html>`;
    ...
    _openCampaignForm() {
        this.showModal(`
            ...
            <div class="form-group"><label>Nome</label><input type="text" id="campName" placeholder="Ex: Promoção Fim de Ano 2026"></div>
            <div class="form-group"><label>Descrição (opcional)</label><input type="text" id="campDesc" placeholder="Notas internas sobre a campanha"></div>
            <div style="display:flex;gap:10px">
                <div class="form-group" style="flex:1"><label>Início</label><input type="date" id="campStart"></div>
                <div class="form-group" style="flex:1"><label>Fim (opcional)</label><input type="date" id="campEnd"></div>
            </div>
            <div style="display:flex;gap:10px">
                <div class="form-group" style="flex:1"><label>Meta de Receita (MZN, opcional)</label><input type="number" id="campGoalRevenue" placeholder="0"></div>
                <div class="form-group" style="flex:1"><label>Meta de Registos (opcional)</label><input type="number" id="campGoalSignups" placeholder="0"></div>
            </div>
            <div class="modal-actions">
                <button style="background:#f1f5f9;color:#0f172a" onclick="adminApp.closeModal()">Cancelar</button>
    ...
    _openGoalForm(metric, currentTarget) {
        ...
        this.showModal(`
            ...
            <div class="form-group"><label>Valor alvo</label><input type="number" id="goalTarget" value="${currentTarget || ''}" placeholder="Ex: 50000"></div>
