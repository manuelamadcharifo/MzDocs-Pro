...
USING (EXISTS (
  ...
  WHERE id = auth.uid() AND is_admin = TRUE
    ...
    // NOVO: Programa de Afiliados — estas chaves (aff_*) já existiam na base
    // de dados desde a migração v10/v14/v36, mas não tinham campo no painel;
    // eram só editáveis directamente por SQL. Reaproveita o mesmo endpoint
    // genérico /api/admin/settings (aceita qualquer chave em system_settings).
    async _loadAffSettings() {
        const loader = document.getElementById('affSettingsLoader');
        const form   = document.getElementById('affSettingsForm');
        if (!loader || !form) return;
        ...
        try {
            ...
            set('cfg_aff_rate_avulso',       'aff_rate_avulso');
            set('cfg_aff_rate_starter',      'aff_rate_starter');
            ...
            loader.textContent = '⚠️ Erro ao carregar configurações de afiliados: ' + err.message;
            console.error('[Admin] _loadAffSettings:', err);
        }
    }
    ...
    async saveAffiliateSettings() {
        ...
        const updates = {
            ...
            aff_rate_avulso:        get('cfg_aff_rate_avulso'),
            aff_rate_starter:       get('cfg_aff_rate_starter'),
        ...
        try {
            ...
            const data = await res.json();
            if (!res.ok) throw new Error(data.error || 'Erro ao guardar');
            this._notify('✅ Programa de afiliados guardado com sucesso!', 'success');
        } catch (err) {
            this._notify('❌ ' + err.message, 'error');
        }
    }
            ...
            container.innerHTML = data.map(tpl => {
                ...
                return `<div style="background:#fff;border:1.5px solid #e2e8f0;border-radius:16px;overflow:hidden;box-shadow:0 2px 12px rgba(0,0,0,.07)">
                  ...
                  <div style="padding:14px">
                    ...
                    <!-- Preço em créditos (uso normal na plataforma) -->
                    <div style="display:flex;align-items:center;gap:8px;margin-bottom:8px;background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;padding:8px 10px">
                    ...
                    <!-- v38: preço em MZN + repartição com o criador (60-70% para ele, resto para a plataforma) -->
                    <div style="display:flex;align-items:center;gap:8px;margin-bottom:10px;background:#f0fdf4;border:1px solid #bbf7d0;border-radius:8px;padding:8px 10px;flex-wrap:wrap">
                      <label style="font-size:11px;font-weight:700;color:#166534;white-space:nowrap">💰 Preço (MZN):</label>
                      <input type="number" id="tplPrice-${tpl.id}" value="${tpl.price_mzn || 0}" min="0" step="0.01"
                        style="width:70px;padding:4px 6px;border:1px solid #86efac;border-radius:6px;font-size:12px;font-weight:700;text-align:center"/>
                      <label style="font-size:11px;font-weight:700;color:#166534;white-space:nowrap;margin-left:6px">% para o criador:</label>
                      ...
                      <span style="font-size:10px;color:#166534">(60-70%; resto fica para a plataforma)</span>
    ...
    // plataforma), o destaque, e — v38 — o preço em MZN e a percentagem
    // que fica para o criador quando outro utilizador usa o template
    // (sempre entre 60% e 70%, validado aqui e também no servidor/BD).
    async _saveTemplatePricing(id) {
        ...
        const priceInput = document.getElementById(`tplPrice-${id}`);
        ...
        if (!Number.isFinite(price) || price < 0) {
            this._notify('Preço em MZN deve ser 0 ou superior.', 'error');
            return;
        }
        ...
        try {
            ...
            const res   = await fetch('/api/admin/templates', {
                ...
                body: JSON.stringify({
                    updates: [{
                        ...
                        price_mzn: price, author_share_percent: share,
                    }],
                }),
            });
            const d = await res.json();
            ...
            this._notify(price > 0 ? `✅ Preço definido: ${price} MZN (${share}% para o criador).` : `✅ Guardado (${cost} crédito(s), sem preço em MZN).`);
        } catch (err) {
