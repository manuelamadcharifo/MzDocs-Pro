...
USING (EXISTS (
  ...
  WHERE id = auth.uid() AND is_admin = TRUE
    ...
    async _approveAffiliate(userId) {
        ...
        alert('✅ Afiliado aprovado! Notificação enviada.');
        this._loadAffiliates();
    }

    async _revokeAffiliate(userId) {
        if (!confirm('Revogar aprovação deste afiliado?')) return;
        const token = await this._getAdminToken();
        const res = await fetch('/api/admin/affiliates', {
            method: 'POST',
            headers: { Authorization: 'Bearer ' + token, 'Content-Type': 'application/json' },
    ...
    async _unblockAffiliate(userId) {
        if (!confirm('Reactivar esta conta?')) return;
        const token = await this._getAdminToken();
        const res = await fetch('/api/admin/affiliates', {
            method: 'POST',
            headers: { Authorization: 'Bearer ' + token, 'Content-Type': 'application/json' },
    ...
    _viewAffiliate(userId) {
        ...
        content.innerHTML = `
          ...
          <div style="display:flex;flex-direction:column;gap:8px">
            ...
            ${!aff.aff_is_blocked
              ? `<button onclick="(function(){const r=prompt('Motivo da suspensão:');if(r)adminApp._blockAffiliate('${aff.id}',r)})()" style="background:#fef2f2;color:#991b1b;border:1.5px solid #fca5a5;border-radius:10px;padding:11px;font-size:13px;font-weight:700;cursor:pointer;width:100%">⛔ Suspender Conta</button>`
    ...
    async _processWithdrawal(wdId, status) {
        const labels = { completed: 'Confirmar pagamento via M-Pesa?', rejected: 'Motivo da rejeição (deixe vazio para omitir):' };
        let note = null;
        if (status === 'rejected') {
            note = prompt(labels.rejected) || '';
            if (note === null) return; // cancelado
        } else {
            if (!confirm(labels.completed)) return;
        }
        const token = await this._getAdminToken();
        const res = await fetch('/api/admin/affiliates', {
            method: 'POST',
        ...
        const d = await res.json();
        if (!res.ok) { alert('Erro: ' + d.error); return; }
        alert(status === 'completed' ? '✅ Pagamento confirmado! Afiliado notificado.' : '❌ Levantamento rejeitado. Saldo devolvido.');
        this._loadWithdrawals(status === 'completed' ? 'pending' : 'pending');
    }
    ...
    async _generateRanking() {
        const month = prompt('Mês a gerar (YYYY-MM):', new Date().toISOString().slice(0, 7));
