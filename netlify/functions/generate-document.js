...
exports.handler = async (event) => {
  ...
  if (!OPENROUTER_KEY) {
    ...
    return { statusCode: 503, headers, body: JSON.stringify({ error: 'Serviço indisponível. Configure OPENROUTER_API_KEY.' }) };
  }

  let body;
  try { body = JSON.parse(event.body || '{}'); }
  catch { return { statusCode: 400, headers, body: JSON.stringify({ error: 'Body JSON inválido' }) }; }
  ...
  if (typeof userCredits === 'number' && userCredits < 1) {
    return { statusCode: 402, headers, body: JSON.stringify({ error: 'INSUFFICIENT_CREDITS', code: 'INSUFFICIENT_CREDITS' }) };
  }
