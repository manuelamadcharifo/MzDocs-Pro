...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (event.httpMethod !== 'POST') {
      return ErrorHandler.createResponse(405, 'Method Not Allowed');
    }
    ...
    try {
      ...
      return ErrorHandler.createResponse(400, 'Body inválido');
    }
    ...
    if (environment !== serverEnv) {
      return ErrorHandler.createResponse(
        400,
        `Ambiente incorreto. Esperado: ${serverEnv}, Recebido: ${environment}`
      );
    }
    ...
    if (parseInt(amount) !== pkg.amount) {
      return ErrorHandler.createResponse(400, 'Montante não corresponde ao pacote');
    }
    ...
    if (!serverHasCreds) {
      ...
      return ErrorHandler.createResponse(
        503,
        'Pagamentos indisponíveis. Configure as credenciais M-Pesa.'
      );
    }
