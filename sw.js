...
const CACHE_VERSION = 'v32-20260825'; // NOVO: subido de v31-20260818 — agendamento real de foto/impressão com papelarias (ver ServiceDefinitions.js, NearbyPartners.js, DocumentController.js, api/partners.js e parceiro-portal.html); sem invalidar o cache antigo, telemóveis com a PWA já instalada continuariam a ver o formulário sem os campos de telefone/data/hora e sem a aba "Marcações" no Portal — o precache abaixo lista index.html, OCRController.js, DocumentController.js e styles.css com `revision: CACHE_VERSION`; para o Workbox, mesma URL + mesma revisão = "ficheiro igual ao anterior", por isso NUNCA ia buscar as versões novas desses ficheiros ao servidor, mesmo depois de um deploy — o site continuava a servir a app antiga do cache local em todos os telemóveis já com a PWA instalada/visitada antes. Subir este valor invalida esse cache e força o download das versões actuais (fluxo do acumulador de páginas do OCR — ver OCRController.js). CORRIGIDO (anterior): 'activate' apagava a cache REAL do precache (CSS/JS/imagens) a cada deploy, porque comparava com uma string literal errada em vez do nome real (workbox.core.cacheNames.precache). Por isso a app abria offline mas sem nenhum estilo.
