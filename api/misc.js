...
async function tplSubmit(req, res, supabase) {
  ...
  // NOVO (v38): o cliente pode propor as suas próprias regras de venda —
  // o preço (em MZN) a cobrar de quem usar o seu template, e a
  ...
  // CHECK constraint, como segunda camada de protecção.
  const rawPrice = parseFloat(body.price_mzn);
  const price_mzn = Number.isFinite(rawPrice) && rawPrice >= 0 ? rawPrice : 0;
  ...
  return res.status(201).json({
    ...
    message: price_mzn > 0
      ? `Template submetido com preço de ${price_mzn} MZN (${author_share_percent}% para si). Aguarda aprovação.`
...
async function tplUse(req, res) {
  ...
  try {
    ...
    // NOVO (v38): repartição de receita entre o criador do template
    ...
    // constraint na base de dados). Só corre para utilizadores autenticados
    // a usar um template de outra pessoa; sem efeito em templates
    // gratuitos ou no próprio autor a usar o seu template (a RPC trata
    // isso e devolve split:false nesses casos).
    if (result?.success && user?.id) {
      rpc('process_template_sale', {
        p_template_id: template_id,
        p_buyer_id: user.id,
        p_credits_spent: result.credit_cost || 0,
