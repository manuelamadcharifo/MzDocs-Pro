// api/misc.js — v2.0 (AUDITORIA Junho/2026)
// ALTERAÇÕES v2.0:
//  1. Removido @supabase/supabase-js + require('ws') do makeClient() interno.
//     Funções que só precisam da service_role usam api/_lib/supabaseAdmin.js.
//     Funções que ainda dependem do SDK (afiliados, templates) mantêm uma instância
//     local criada APENAS quando necessária, sem ws (usa fetch nativo do Node 20).
//  2. SEGURANÇA (C-1): handleConfig() deixa de expor supabaseAnonKey no JSON.
//     O frontend já tem a chave configurada em deploy; expô-la em /api/config
//     era redundante e aumentava a superfície de ataque.
//  3. Contador de documentos usa a tabela analytics_metrics pré-agregada quando
//     disponível, evitando COUNT(*) full-scan em credit_usage_log.
