...
// ─────────────────────────────────────────────────────────────────────────────
// CORRIGIDO: esta função tinha o mesmo nome (_slugify) da já existente mais
// acima (usada pelos slugs de blog) — duas declarações `function` com o
// mesmo identificador no mesmo módulo são um SyntaxError ("Identifier has
// already been declared"), o que impedia api/admin/index.js de sequer
// carregar (derrubava TODO o painel de admin, não só os QR codes).
// Renomeada para _qrSlug; mantém exactamente o comportamento original
// (underscore, 30 caracteres, fallback 'qr') — só o nome mudou.
function _qrSlug(s) {
  return (s || '')
    .toLowerCase()
