  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      let q = supabase.from('templates_custom')
        ...
        .select('id, user_id, service_type, template_name, description, template_type, status, is_featured, is_public, credit_cost, price_mzn, author_share_percent, use_count, downloads, rating_sum, rating_count, rejection_note, template_html, template_css, created_at')
      ...
      const templates = (data || []).map(t => {
        ...
        return { ...rest, avg_rating, complexity_score };
      });
    ...
    if (req.method === 'PUT' || req.method === 'POST') {
      const body    = parseBody(req);
      const updates = body?.updates; // [{ id, credit_cost?, is_featured?, status?, template_type?, price_mzn?, author_share_percent? }]
      ...
      // faltavam rejection_note (usado ao rejeitar) e price_mzn/
      // author_share_percent (novos campos da repartição de vendas com o
      // criador do template).
      const ALLOWED_FIELDS = ['credit_cost', 'is_featured', 'status', 'template_type', 'is_public', 'rejection_note', 'price_mzn', 'author_share_percent'];
      const results = [];
      for (const u of updates) {
        ...
        if (patch.price_mzn !== undefined) {
          const p = parseFloat(patch.price_mzn);
          if (!Number.isFinite(p) || p < 0) { results.push({ id: u.id, ok: false, error: 'price_mzn inválido (>= 0)' }); continue; }
          patch.price_mzn = p;
        }
